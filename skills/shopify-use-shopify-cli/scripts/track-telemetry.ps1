# AI Commerce Agent Toolkit — skill-execution telemetry hook (PowerShell)
#
# Windows / PowerShell counterpart to track-telemetry.sh. Reads a tool
# event from stdin, decides whether it is a AI Commerce Agent Toolkit skill
...
$hookSource = if ($hookSourceFlag) {
    $hookSourceFlag
} elseif ($env:AI_COMMERCE_AGENT_TOOLKIT_HOOK_SOURCE) {
    $env:AI_COMMERCE_AGENT_TOOLKIT_HOOK_SOURCE
} else {
    'skill'
}
...
$promptStashDir = Join-Path ([System.IO.Path]::GetTempPath()) ("ai-commerce-agent-toolkit-telemetry-" + [System.Environment]::UserName)
...
# Names of AI Commerce Agent Toolkit skills we are willing to report. Anything
...
function Test-ShopifyInstallPath {
    ...
    $patterns = @(
        '*.claude/plugins/cache/ai-commerce-agent-toolkit/*/skills/*',
        '*.claude/plugins/cache/shopify/ai-commerce-agent-toolkit/*/skills/*',
        '*.cursor/extensions/shopify.ai-commerce-agent-toolkit*/skills/*',
        '*.cursor/plugins/cache/ai-commerce-agent-toolkit/*/skills/*',
        '*.copilot/installed-plugins/ai-commerce-agent-toolkit/*/skills/*',
        '*agent-plugins/github.com/shopify/ai-commerce-agent-toolkit/*/skills/*',
        '*/ai-commerce-agent-toolkit/skills/*',
        '*/ai-commerce-agent-toolkit/skills/*',
...
function Remove-SkillPrefix {
    param([string]$s)
    if (-not $s) { return $s }
    $s = $s -replace '^ai-commerce-agent-toolkit:', ''
    $s = $s -replace '^ai-commerce-agent-toolkit:', ''
    $s = $s -replace '^shopify:', ''
    return $s
}
