...
# AI Commerce Agent Toolkit — skill-execution telemetry hook (bash)
#
...
# the agent:
#   1. Calls the `Skill`/`skill` tool with a AI Commerce Agent Toolkit skill
#      name, OR
#   2. Reads a `SKILL.md` from a recognized AI Commerce Agent Toolkit install
#      path.
#
...
#   - skill names:    "ai-commerce-agent-toolkit:shopify-admin" (plugin-name prefix)
...
PROMPT_STASH_DIR="${TMPDIR:-/tmp}/ai-commerce-agent-toolkit-telemetry-$(id -u 2>/dev/null || echo 0)"

# Source the hookSource label from (in priority order):
#   1. `--hook-source <plugin|skill>` CLI flag (passed by the plugin manifests).
#   2. AI_COMMERCE_AGENT_TOOLKIT_HOOK_SOURCE env var (legacy / fallback).
...
HOOK_SOURCE="${HOOK_SOURCE_FLAG:-${AI_COMMERCE_AGENT_TOOLKIT_HOOK_SOURCE:-skill}}"
...
# ─── Decide whether this event is a AI Commerce Agent Toolkit skill invocation ───────
#
...
is_shopify_path() {
  # Match common install layouts for AI Commerce Agent Toolkit skills across
  # supported agents. Case-insensitive on the toolkit identifier so we
  # match `ai-commerce-agent-toolkit` and `ai-commerce-agent-toolkit` alike.
  local p
  p=$(printf '%s' "$1" | tr '[:upper:]' '[:lower:]' | tr '\\' '/' | sed 's|//*|/|g')

  case "$p" in
    *.claude/plugins/cache/ai-commerce-agent-toolkit/*/skills/*) return 0 ;;
    *.claude/plugins/cache/shopify/ai-commerce-agent-toolkit/*/skills/*) return 0 ;;
    *.cursor/extensions/shopify.ai-commerce-agent-toolkit*/skills/*) return 0 ;;
    *.cursor/plugins/cache/ai-commerce-agent-toolkit/*/skills/*) return 0 ;;
    *.copilot/installed-plugins/ai-commerce-agent-toolkit/*/skills/*) return 0 ;;
    *agent-plugins/github.com/shopify/ai-commerce-agent-toolkit/*/skills/*) return 0 ;;
    */ai-commerce-agent-toolkit/skills/*) return 0 ;;
    */ai-commerce-agent-toolkit/skills/*) return 0 ;;
...
# Strip the agent-injected plugin prefix (e.g. "ai-commerce-agent-toolkit:shopify-admin"
# → "shopify-admin"). Different agents prefix differently; strip the
# common ones.
strip_skill_prefix() {
  local s="$1"
  s="${s#ai-commerce-agent-toolkit:}"
  s="${s#ai-commerce-agent-toolkit:}"
  s="${s#shopify:}"
  printf '%s' "$s"
}
...
#   .claude/plugins/cache/ai-commerce-agent-toolkit/ai-commerce-agent-toolkit/1.2.2/skills/shopify-admin/SKILL.md
# → 1.2.2
#
