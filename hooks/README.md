...
1. Calls the host agent's `Skill` / `skill` tool with a Shopify AI
   ...
   `shopify-liquid`).
2. Reads a `SKILL.md` file from a recognized AI Commerce Agent Toolkit install
   path.
...
- Plugin manifests prefix the hook command with
  `AI_COMMERCE_AGENT_TOOLKIT_HOOK_SOURCE=plugin`. The skill-frontmatter
...
(`${TMPDIR:-/tmp}/ai-commerce-agent-toolkit-telemetry-$(id -u)/`), written `0600`
...
- Any tool call whose target is not a AI Commerce Agent Toolkit skill or
...
AI_COMMERCE_AGENT_TOOLKIT_HOOK_SOURCE=plugin \
echo '{"hook_event_name":"PostToolUse","tool_name":"Skill","tool_input":{"skill":"ai-commerce-agent-toolkit:shopify-admin"},"session_id":"local","tool_use_id":"toolu_local"}' \
